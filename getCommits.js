// ==UserScript==
// @name         GitLab Commits 采集助手 (带关闭按钮版)
// @namespace    http://tampermonkey.net/
// @version      2.2
// @description  自动翻页采集，支持展开描述，去除特殊符号，带 UI 关闭功能
// @author       Gemini
// @match        */*/commits/*
// @grant        none
// ==UserScript==
...
(function() {
    ...
    panel.id = 'gl-scraper-container'; // 给容器一个ID方便操作
    panel.innerHTML = `
        <div id="gl-scraper-panel" style="position:fixed; top:80px; right:20px; z-index:9999; background:#fff; border:1px solid #ddd; border-radius:12px; padding:15px; box-shadow:0 8px 24px rgba(0,0,0,0.2); width:240px; font-family:system-ui, -apple-system, sans-serif;">
            ...
            <div style="margin-bottom:12px;">
                ...
                <input type="month" id="target-month" style="width:100%; padding:6px; border:1px solid #dcdfe6; border-radius:4px;" value="2026-01">
            </div>
    ...
    // --- 2. 交互逻辑：关闭按钮 ---
    closeBtn.onclick = () => {
        if (confirm('确认关闭采集助手？（关闭后需刷新页面才能重新开启）')) {
            panel.remove();
        }
    };
    ...
    startBtn.onclick = async () => {
        ...
        try {
            while (true) {
                ...
                if (isPastTarget) {
                    statusMsg.innerHTML += '<br><b style="color:#f5222d;">检测到日期已超出范围。</b>';
                    break;
                }
    ...
    async function scrapeCurrentPage(targetMonth) {
        ...
        for (const header of dateHeaders) {
            ...
            if (day < targetMonth && !day.startsWith(targetMonth)) {
                isPastTarget = true;
                continue;
            }
